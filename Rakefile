...
repo_url = "https://github.com/w3cub/w3cub-release-202603.git"
...
task :badlink do |t, args|
  scount = 0
  log_file = File.open("badlink_output.log", "w")

  Dir.glob("#{docs_cache_dir}/**/*.html") do |dir|
    ...
    if (arr[0] && arr[0].index("_") == 0) || dir.include?("...")
      ...
      line = dir.sub("\.docs-cache\/", " - ")

      puts line                # 控制台输出（可选）
      log_file.puts(line)      # 写入文件
    end
  end

  summary = "total: #{scount}"
  puts summary
